...
exec "${DOCKER:?}" run --tty --interactive --rm \
	...
	${MUSIC_FOLDER+ \
		--mount type=bind,src="${MUSIC_FOLDER:?}",dst='/home/wine/Music',ro \
	} \
	${X11_SOCKET_DIRECTORY+ \
		...
		${DEVICE_DRI+ \
			--device "${DEVICE_DRI:?}":'/dev/dri':r \
		} \
		${DEVICE_NVIDIACTL+ \
			--device "${DEVICE_NVIDIACTL:?}":'/dev/nvidiactl':r \
		} \
		${DEVICE_NVIDIA0+ \
			--device "${DEVICE_NVIDIA0:?}":'/dev/nvidia0':r \
		} \
	} \
	${PULSEAUDIO_SOCKET+ \
